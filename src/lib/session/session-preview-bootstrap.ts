...
type SessionPreviewBootstrapOptions = {
  errorTag: string;
};
...
export async function handlePostSessionPreviewBootstrap(
  req: NextRequest,
  options: SessionPreviewBootstrapOptions
) {
  try {
    ...
    if (active?.session_number) {
      ...
      const planJson = row.plan_json as {
        ...
        segments?: SessionPreviewRouteData['segments'];
      } | null;
      const phase =
      ...
      return ok<SessionPreviewRouteData>({
