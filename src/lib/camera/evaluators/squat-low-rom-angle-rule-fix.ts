...
function getHipAngleAvg(frame: PoseFeaturesFrame): number | null {
  ...
  const right =
    frame.joints.rightShoulder && frame.joints.rightHip && frame.joints.rightKnee
      ...
      : null;
  return meanSafe(
    [left, right].filter((v): v is number => typeof v === 'number' && Number.isFinite(v))
  );
}

function readPromotionDepth(frame: PoseFeaturesFrame): number | null {
  const primary = frame.derived.squatDepthProxy;
  if (typeof primary === 'number' && Number.isFinite(primary)) return primary;
  const blended = frame.derived.squatDepthProxyBlended;
  return typeof blended === 'number' && Number.isFinite(blended) ? blended : null;
}
...
export function detectLowRomAngleRuleCycle(
  ...
  if (state == null || state.baselineFrozen !== true || state.peakLatched !== true) return null;
  ...
  if (prePeak.length < LOW_ROM_MIN_PRE_PEAK_FRAMES || postPeak.length < LOW_ROM_MIN_POST_PEAK_FRAMES) {
    return null;
  }
  ...
  if (baselineKnee == null || baselineHip == null || peakKnee == null || peakHip == null) {
    return null;
  }
  ...
  const requiredKneeRecovery = Math.max(
    LOW_ROM_MIN_KNEE_FLEX_DEG,
    kneeFlexion * LOW_ROM_EXTENSION_RATIO
  );
  const requiredHipRecovery = Math.max(
    LOW_ROM_MIN_HIP_FLEX_DEG,
    hipFlexion * LOW_ROM_EXTENSION_RATIO
  );
