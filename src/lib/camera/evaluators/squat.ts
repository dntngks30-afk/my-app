...
function buildSquatEvaluatorHighlightedMetrics(p: {
  ...
  return {
    ...
    latestSquatDepthProxy: p.latestSquatDepthProxy ?? null,
    ...
    attemptStarted: state.attemptStarted ?? false,
    currentSquatPhase: state.currentSquatPhase ?? null,
    descendConfirmed: state.descendConfirmed ?? false,
    committedAtMs: state.committedAtMs ?? null,
    ascendConfirmed: state.ascendConfirmed ?? false,
    standingRecoveredAtMs: state.standingRecoveredAtMs ?? null,
    standingRecoveryHoldMs: state.standingRecoveryHoldMs ?? null,
    standingRecoveryFrameCount: state.standingRecoveryFrameCount ?? null,
    standingRecoveryThreshold: Math.round(state.standingRecoveryThreshold * 100) / 100,
    standingRecoveryMinFramesUsed: state.standingRecoveryMinFramesUsed ?? null,
    standingRecoveryMinHoldMsUsed: state.standingRecoveryMinHoldMsUsed ?? null,
    standingRecoveryBand: state.standingRecoveryBand ?? null,
    standingRecoveryFinalizeReason: state.standingRecoveryFinalizeReason ?? null,
    ...
    evidenceLabel: state.evidenceLabel ?? null,
    completionBlockedReason: state.completionBlockedReason ?? null,
    completionSatisfied: state.completionSatisfied ?? false,
    ...
    completionMachinePhase: state.completionMachinePhase ?? null,
    completionPassReason: state.completionPassReason ?? null,
    ...
    recoveryReturnContinuityFrames: state.recoveryReturnContinuityFrames ?? null,
    recoveryTrailingDepthCount: state.recoveryTrailingDepthCount ?? null,
    recoveryDropRatio:
      state.recoveryDropRatio != null
        ? Math.round(state.recoveryDropRatio * 100) / 100
        : null,
    lowRomRecoveryReason: state.lowRomRecoveryReason ?? null,
    ultraLowRomRecoveryReason: state.ultraLowRomRecoveryReason ?? null,
    repCount: p.repCountEstimate,
    cycleComplete: state.cycleComplete ? 1 : 0,
    startBeforeBottom: state.startBeforeBottom ? 1 : 0,
    descendStartAtMs: state.descendStartAtMs ?? null,
    peakAtMs: state.peakAtMs ?? null,
    reversalAtMs: state.reversalAtMs ?? null,
    ascendStartAtMs: state.ascendStartAtMs ?? null,
    recoveryAtMs: state.standingRecoveredAtMs ?? null,
    cycleDurationMs: state.cycleDurationMs ?? null,
