 ...
 * knee / hip angle 변화로 한 번 더 홑m��인한다.
 *
 * - low-rom 대역(0.07 <= relativeDepthPeak < 0.4)k��읔 대상
 * - standing / descent-only / ultra-low / trajectory rescue / event promotion 아보고 쑬혗
 * - deep standard path는 기조 wrapper 겼과를 그대로 사용
 */
...
function findPeakIndex(valid: PoseFeaturesFrame[], state: SquatCompletionState): number {
  if (
    ...
    state.peakLatchedAtIndex < kalid.length
  ) {
