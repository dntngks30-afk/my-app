...
export const BASELINE_STEP1_RESULT_SLOTS: Record<UnifiedPrimaryType, BaselineStep1ResultSlots> = {
  LOWER_INSTABILITY: {
    ...
    heroCoreLines: [
      `당신의 움직임 타입은
오래 앉아 있거나, 한쪽 다리에 기대 서 있거나,
준비 없이 갑자기 일어나 움직이는 습관에서
비롯되었을 수 있습니다.
...
익숙해지기 쉽습니다.
그래서 쪼그려 앉았다 일어나거나
계단을 오르내릴 때 중심이 한쪽으로 흔들리는
느낌이 먼저 나타날 수 있습니다.

지금은 발바닥·발목·엉덩이가 함께
체중을 받쳐주는 감각을 다시 만드는 것이 중요합니다.`,
    ],
...
컴퓨터나 스마트폰을 오래 보는 자세,
고개를 앞으로 내민 습관,
그리고 가슴 위쪽으로 짧게 숨 쉬는 패턴에서
비롯되었을 수 있습니다.

이런 상태가 계속되면 목과 어깨 앞쪽, 가슴 주변이
먼저 굳고, 등과 견갑은 부드럽게 움직이지 못한 채
상체 전체가 긴장으로 버티게 됩니다.
그래서 팔을 들거나 옷을 갈아입고
머리를 감을 때 어깨가 먼저 답답하고,
오래 앉아 있을수록 목과 어깨가
쉽게 무거워질 수 있습니다.

지금은 자세를 억지로 세워보기보다
숨을 편하게 만들고, 가슴·등·어깨가 함께 움직이도록
긴장을 먼저 풀어주는 것이 중요합니다.`,
    ],
  ...
  CORE_CONTROL_DEFICIT: {
    ...
    heroCoreLines: [
      `당신의 움직임 타입은 오래 앉아 있는 생활,
몸통 전체보다 허리만 세워 버티는 습관,
그리고 숨을 편하게 쉬기보다
배나 허리를 먼저 조이는 패턴에서
비롯되었을 수 있습니다.

이런 상태가 반복되면
배·갈비·골반이 함께 중심을 잡기보다
허리와 골반이 먼저 긴장해 몸통을 대신 지탱하는
방식이 굳어지기 쉽습니다.
그래서 허리를 숙이거나 방향을 바꿀 때
몸통이 부드럽게 이어지기보다
허리와 골반이 먼저 뻣뻣해지고,
움직일수록 허리가 더 힘들게 느껴질 수 있습니다.

지금은 배에 힘을 더 주는 것보다,
호흡과 함께 몸통 중심이 자연스럽게 연결되도록
다시 정리하는 것이 먼저입니다.`,
    ],
  ...
  DECONDITIONED: {
    ...
    heroCoreLines: [
      `당신의 움직임 타입은
운동 공백이 길었거나, 수면과 회복이 불규칙하고,
활동량이 들쭉날쭉한 생활 패턴에서
비롯되었을 수 있습니다.

이런 흐름이 이어지면 특정 한 부위보다
몸 전체가 쉽게 무겁고 둔하게 느껴지고,
움직임을 시작할 때 필요한
기본 리듬이 떨어지기 쉽습니다.
그래서 조금만 움직여도
여러 부위가 함께 피곤해지거나, 잠깐 괜찮다가도
금방 지치면서 자세가 전체적으로 흐트러질 수 있습니다.

지금은 한 부위를 집중해서 바꾸기보다
몸 전체가 다시 가볍게 움직일 수 있는
기본 체력과 회복 리듬을 되찾는 것이 중요합니다.`,
    ],
    patternToWatch:
  ...
  STABLE: {
    ...
    heroCoreLines: [
      `당신의 움직임 타입은
현재 몸의 큰 흐름이 비교적 잘 정리되어 있고,
한 부위가 과하게 무너져 전체 움직임을
끌고 가는 패턴은 크지 않은 상태에 가깝습니다.

다만 이것이 항상 같은 상태로
유지된다는 뜻은 아니며, 수면 부족이나 피로,
스트레스가 쌓이면 몸의 균형도
생각보다 쉽게 흔들릴 수 있습니다.
그래서 평소에는 편하게 움직여도
컨디션이 떨어진 날에는 특정 부위가 먼저 뻣뻣해지거나
힘이 몰리는 변화가 나타날 수 있습니다.

지금은 크게 고치기보다
이 균형을 유지하면서 몸 상태에 맞춰
강도와 회복을 조절하는 방식이 가장 잘 맞습니다.`,
    ],
    patternToWatch:
...
// ─── PR-BASELINE-STEP3-SCROLL-ACTION-08 — 스크롤 액션 씬 + 실행 브리지 (표시 전용) ─

/** Step3 히어로 바로 아래 한 줄 — 스크롤 씬 톤 */
export const STEP3_ACTION_SCENE_SUBLINE = '오늘 몸 상태를 기준으로, 리셋을 시작하는 순서예요.';

/** 미디어 슬롯 스크린리더 안내 */
export const STEP3_MEDIA_PLACEHOLDER_ARIA = '대표 동작 예시 이미지가 들어갈 자리입니다.';

/** 실행으로 넘어가기 직전 브리지 블록 제목 */
export const STEP3_EXECUTION_BRIDGE_TITLE = '움직임 리셋을 시작하면';

/**
 * 결제·잠금 표현 없이 실행 연속성만 전달 (짧은 불릿).
 */
export const STEP3_EXECUTION_BRIDGE_BULLETS: readonly string[] = [
  '지금 결과는 개인 맞춤 리셋 맵으로 이어져요.',
  '첫 세션은 오늘 시작점을 기준으로 준비돼요.',
  '세션을 마치면 다음 흐름이 이어지도록 정리돼요.',
  '한 번 본 결과로 끝내지 않고, 실행과 완료에 맞춰 설계됐어요.',
];
