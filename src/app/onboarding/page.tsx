 ...
 *
 * 결제 후 실행 준비 입력 수집.
 * session_user_profile에 저장 (target_frequency, exercise_experience_level, pain_or_discomfort_present).
 *
 * ─── 역할 경계 ─────────────────────────────────────────────────────────────────
 * - 실행 준비 입력만 수집 (분석 아님)
 * - claim은 FLOW-05, session create는 FLOW-06
 *
 * ─── Bridge Context ───────────────────────────────────────────────────────────
 * query 또는 localStorage에서 publicResultId/stage 유지 (FLOW-05 claim용).
 * onboarding 저장과 별개로 보존.
 *
...
export default function OnboardingPage() {
  ...
  }, [targetFrequency, exerciseExperienceLevel, painOrDiscomfortPresent, router]);
    ...
    >
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-400" style={{ fontFamily: 'var(--font-sans-noto)' }}>
            맞춤 루틴을 위해 몇 가지를 알려주세요.
          </p>
        </div>

        <div className="space-y-4">
          <div>
            <label className="block text-slate-400 text-xs mb-2" style={{ fontFamily: 'var(--font-sans-noto)' }}>
              주당 운동 목표 횟수 *
            </label>
