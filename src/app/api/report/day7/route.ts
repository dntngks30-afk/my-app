...
export async function GET(req: NextRequest) {
  ...
  if (conditionsRes.error || completionsRes.error) {
    return NextResponse.json(
      { error: '조회 실패', details: conditionsRes.error?.message ?? completionsRes.error?.message },
      { status: 500 }
    );
  }
  ...
  const journey_complete = summary.completed_days >= 7;
