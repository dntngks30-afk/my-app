...
import { getKpiDetails, resolveKpiRange } from '@/lib/analytics/admin-kpi';
import {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await getKpiDetails(admin.supabase, range, pilotCode);
    ...
    const res = NextResponse.json({ ok: false, error: 'KPI_DETAILS_FAILED' }, { status: 500 });
    res.headers.set('Cache-Control', 'no-store');
    return res;
  }
}
