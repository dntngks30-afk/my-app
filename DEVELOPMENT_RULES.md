...
### Contract 6. Selected truth vs realized alignment separation (PR4/PR3 lock)
- `selected_truth_trace`는 what truth won and why를 기록하는 contract다.
- `alignment_audit`는 output이 그 truth를 honor했는지 판정하는 contract다.
- 두 계약을 하나의 설명 블록으로 합치거나 상호 대체하면 안 된다.
- trace cleanup PR과 alignment audit PR을 한 PR에서 의미 재정의로 섞지 않는다.

### Contract 7. Session 1 alignment + phase law
- session 1은 selected current state truth continuation으로 체감되어야 한다.
- `Prep` semantics는 `Main` semantics를 silently dominate하면 안 된다.
- “설명 텍스트 정합”만으로 alignment 달성을 주장하지 않는다.
...
- Do not let public result truth stop at result page; it must continue into session creation truth
...
- Do not conflate `selected_truth_trace` with `alignment_audit`
