...
### 11.1 PR5-FIX-3 이후 최종 실기기 검증 결과 (CURRENT_IMPLEMENTED)
...
| 1 | 얕은 스쿼트 | 정상 통과 |
| 2 | 얕은 스쿼트 | 정상 통과 |
| 3 | 얕은 스쿼트 | 앉기 시작 직후 **조기 통과** 1회 (**Known residual risk**, blocking 아님) |
| 4 | 얕은 스쿼트 | 정상 통과 |
| 5 | 딥 스쿼트 | 정상 통과 |

- 얕은 스쿼트: **4회 중 3회** 정상 통과.
- 딥 스쿼트: **마지막 시도** 정상 통과.
- 3번째 얕은 시도의 조기 통과는 **단일 좁은 원인 미확정**. 원인 **후보**는 상호 배타 아님: V2 rolling-window epoch, `latestValidTs_minus_5000ms` 기반 평가 window, setup/framing translation, low-ROM closure, pose translation / recovery interpretation.

**추가 gate 강화**는 얕은 스쿼트 pass 회귀 위험이 크므로 **PR5에서는 보류** (§11.3).

### 11.2 실기기에서 지속 확인할 사항

1. **셋업 포지셔닝 후 서있는 상태** → overhead reach로 즉시 넘어가면 안 됨
2. **앉기 시작 직후** → 원칙적으로 넘어가면 안 됨 (§11.3 잔여 1건 참고)
...
### 11.3 Known Residual Risk — Early Pass After Squat Start
...
다음을 전제로 PR6 (Legacy Quality Analyzer Demotion) 진행:

1. ✅ 실기기에서 **대부분** false positive(하강 시작 직후 통과) 해소 — **5회 검증 요약 §11.1**; 잔여 1건은 §11.3 **Known residual** (PR5 non-blocker)
2. ✅ 실기기에서 valid shallow/deep squat **광범위** 정상 통과 — §11.1
...
6. ✅ 업로드된 fail01/fail02 동등 케이스에서 `stale_closure_not_at_tail` blockReason으로 fail (합성/골든 회귀)

**PR6 범위 고정:**

- PR6는 §11.3 **잔여 early-pass 리스크를 해결하지 않는다**.
...
LOCKED_DIRECTION: **최종** 실기기 5회 검증(§11.1)과 §11.3 **Known residual**을 전제로, PR6는 **legacy quality/debug demotion**만 수행한다. PR6는 잔여 early-pass를 **고치지 않으며**, V2 pass semantics·timing·epoch·threshold·auto-progression ownership를 **변경하지 않는다**. PR5-FIX-3 acceptance command는 구현 시점에 green이었고, fail01/fail02 클래스는 골든/스모크로 계속 봉인한다.
