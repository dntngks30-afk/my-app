...
On the current Home reset map, some session nodes are rendered outside the visible horizontal bounds of the map card. In practice this creates three user-facing failures:
...
- stacked fallback layout for extremely narrow conditions,
...
Preserve the current alternating left/right rhythm as a **preferred** side only.
It is no longer absolute truth.
...
### Step 8 — stacked fallback for narrow cases
...
- text wraps within a bounded width.
...
- label max width: roughly `96–120px` on mobile,
...
- nodes still alternate left/right in the normal case,
...
3. update `SessionNode` label rendering to support wrapped bounded text,
...
It only ensures that the already-selected session display truth can be rendered inside the visible Home map safely.
...
These are acceptable residual risks. They do not justify keeping the current off-screen failure mode.
...
The correct fix is to keep the path as geometry truth and add a **viewport-safe final placement layer** so every session node is visible, readable, and tappable inside the Home reset map.
