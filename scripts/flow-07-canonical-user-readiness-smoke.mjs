...
console.log('\n[2] getCanonicalUserReadiness 유틸 검증');
...
assert("plan_status === 'active' 확인", util?.includes("=== 'active'"));

// 4. 분석(analysis) 상태 확인
console.log('\n[4] analysis 상태 검증');
assert('getLatestClaimedPublicResultForUser 사용', util?.includes('getLatestClaimedPublicResultForUser'));
assert('loadSessionDeepSummary legacy fallback', util?.includes('loadSessionDeepSummary'));
...
assert('exercise_experience_level 체크', util?.includes('exercise_experience_level'));
assert('pain_or_discomfort_present 체크', util?.includes('pain_or_discomfort_present'));
...
assert('FREQUENCY_REQUIRED', util?.includes("'FREQUENCY_REQUIRED'"));
assert('INACTIVE_PLAN', util?.includes("'INACTIVE_PLAN'"));
assert('getTodayCompletedAndNextUnlock 사용', util?.includes('getTodayCompletedAndNextUnlock'));
...
assert('deriveNextAction 함수', util?.includes('function deriveNextAction'));
...
assert('미인증 → UNAUTHENTICATED_READINESS 반환 (401 아님)', route?.includes('UNAUTHENTICATED_READINESS'));
assert('force-dynamic', route?.includes("force-dynamic"));
assert('ok() 사용', route?.includes('return ok('));
assert('getCanonicalUserReadiness 호출', route?.includes('getCanonicalUserReadiness'));

// 9. READ ONLY 검증: DB 변경 없음
console.log('\n[9] READ ONLY 검증');
assert('claim 로직 없음', !util?.includes('claimPublicResult') && !util?.includes('claimed_at ='));
assert('session create 로직 없음', !util?.includes('INSERT INTO session') && !util?.includes('createSession'));
assert('onboarding save 로직 없음', !util?.includes('applyTargetFrequency') && !util?.includes('.upsert('));

// 10. FLOW-08 준비: 재사용 가능 계약
console.log('\n[10] FLOW-08 준비 확인');
assert('next_action.href 존재', util?.includes('href:'));
assert('CanonicalUserReadiness 단일 export (재사용 가능)', util?.includes('export interface CanonicalUserReadiness'));
assert('병렬 DB 조회 (Promise.all)', util?.includes('Promise.all'));
