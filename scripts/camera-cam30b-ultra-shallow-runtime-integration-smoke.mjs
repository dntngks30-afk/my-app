/**
 * PR-CAM-30B  ultra-shallow [0.02,0.08) runtime ?? (auto-progression + timing)
 *
...
console.log('\n?? A. shallow down-up-recover -> pass ??');
{
  const stepMs = 120;
  ...
  ok(
    'A1: completion + not no_reversal',
    cs?.completionSatisfied === true && cs?.completionBlockedReason !== 'no_reversal',
    { block: cs?.completionBlockedReason, relPeak: cs?.relativeDepthPeak }
  );
  ok('A2: gate pass', gate.status === 'pass', gate.status);
  ok('A3: final latch', isFinalPassLatched('squat', gate) === true, gate.finalPassBlockedReason);
}

console.log('\n?? B. descend only -> no pass ??');
{
  const stepMs = 100;
...
console.log('\n?? C. mid-ascent -> no final latch ??');
{
  ...
  const gate = evaluateExerciseAutoProgress('squat', lm, squatStats(lm.length, stepMs));
  ok('C1: latch false', isFinalPassLatched('squat', gate) === false, {
    latch: isFinalPassLatched('squat', gate),
  });
}

console.log('\n?? D. deep squat regression ??');
{
  const stepMs = 120;
  const downUp = [
  ...
  ok(
    ...
    { status: gate.status, rawPeak: cs?.rawDepthPeak }
  );
}

console.log('\n?? E. 0.08~0.12 shallow relax regression ??');
{
  const stepMs = 120;
  const modDown = [
  ...
  const cs = gate.evaluatorResult.debug?.squatCompletionState;
  ok(
    'E1: shallow relax band ok',
    cs?.completionSatisfied === true && cs?.completionBlockedReason !== 'no_reversal',
    { block: cs?.completionBlockedReason, relPeak: cs?.relativeDepthPeak }
  );
}

console.log(`\n=== PR-CAM-30B runtime integration smoke: ${passed} passed, ${failed} failed ===`);
